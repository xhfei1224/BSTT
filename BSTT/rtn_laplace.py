...
for i in range(othercfg['fold']):
    ...
    for fid in range(othercfg['fold']):
        if fid!=4 - i:
            ...
            valX = np.load(othercfg['data_path'] + 'valX_F' + str(fid) + '.npy')
            valY = np.load(othercfg['data_path'] + 'valY_F' + str(fid) + '.npy')
    ...
    plt.subplot(1, 2, 1)
    plt.title('S-Summary graph')
    S_summ_mean = S_summ.mean(axis=0)#keepdims=True
    plt.imshow(row2matrix(S_summ_mean, learning['window_size']))
    plt.colorbar()
    plt.subplot(1, 2, 2)
    plt.title('S-Specific graph')
    S_spec_mean = S_spec.mean(axis=0)#keepdims=True
    plt.imshow(row2matrix(S_spec_mean, learning['window_size']))
    plt.colorbar()
    plt.tight_layout()
    plt.savefig(othercfg['out_dir'] + 'S_graph'+str(i)+'.png')
    plt.show()

    plt.figure(figsize=(10, 4))
    plt.subplot(1, 2, 1)
    plt.title('P-Summary graph')
    P_summ_mean = P_summ.mean(axis=0)#keepdims=True
    plt.imshow(row2matrix(P_summ_mean, learning['time_series']))
    plt.colorbar()
    plt.subplot(1, 2, 2)
    plt.title('P-Specific graph')
    P_spec_mean = P_spec.mean(axis=0)#keepdims=True
    plt.imshow(row2matrix(P_spec_mean, learning['time_series']))
    plt.colorbar()
    plt.tight_layout()
    plt.savefig(othercfg['out_dir'] + 'P_graph'+str(i)+'.png')
    plt.show()
...
S_summ_graph_list = np.concatenate(S_summ_graph_list)
S_spec_graph_list = np.concatenate(S_spec_graph_list)
P_summ_graph_list = np.concatenate(P_summ_graph_list)
P_spec_graph_list = np.concatenate(P_spec_graph_list)
...
####################### Save graph edges #######################
np.save(othercfg['out_dir']+'S_graph_summary.npy',S_summ_graph_list)
np.save(othercfg['out_dir']+'S_graph_specific.npy',S_spec_graph_list)
np.save(othercfg['out_dir']+'P_graph_summary.npy',P_summ_graph_list)
np.save(othercfg['out_dir']+'P_graph_specific.npy',P_spec_graph_list)

plt.figure(figsize=(10, 4))
plt.subplot(1, 2, 1)
plt.title('Summary graph')
S_summ_mean = S_summ_graph_list.mean(axis=0)#keepdims=True
plt.imshow(row2matrix(S_summ_mean, learning['window_size']))
plt.colorbar()
plt.subplot(1, 2, 2)
plt.title('Specific graph')
S_spec_mean = S_spec_graph_list.mean(axis=0)#keepdims=True
plt.imshow(row2matrix(S_spec_mean, learning['window_size']))
plt.colorbar()
plt.tight_layout()
plt.savefig(othercfg['out_dir'] + 'S_graph.png')
plt.show()

plt.figure(figsize=(10, 4))
plt.subplot(1, 2, 1)
plt.title('Summary graph')
P_summ_mean = P_summ_graph_list.mean(axis=0)#keepdims=True
plt.imshow(row2matrix(P_summ_mean, learning['time_series']))
plt.colorbar()
plt.subplot(1, 2, 2)
plt.title('Specific graph')
P_spec_mean = P_spec_graph_list.mean(axis=0)#keepdims=True
plt.imshow(row2matrix(P_spec_mean, learning['time_series']))
plt.colorbar()
plt.tight_layout()
plt.savefig(othercfg['out_dir'] + 'P_graph.png')
plt.show()
