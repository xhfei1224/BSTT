...
def train_epoch(model, train_loader, learning, my_loss, optimizer, epoch, threshold=10):
    ...
    for batch_idx, (x, y) in enumerate(train_loader):
        ...
        loss_kl1 = learning['lamada1'] * result_dic['S_kl_l']
        loss_kl2 = learning['lamada2'] * result_dic['S_kl_b']
        loss_kl3 = learning['lamada3'] * result_dic['P_kl_l']
        loss_kl4 = learning['lamada4'] * result_dic['P_kl_b']
        loss = my_loss(result_dic['ht_output'], y) + loss_kl1 + loss_kl2 + loss_kl3 + loss_kl4
        #print(loss)
        loss.backward()
        ...
        loss_kl3_list.append(loss_kl3.item())
        loss_kl4_list.append(loss_kl4.item())
    ...
    loss_kl3_mean = np.mean(loss_kl3_list)
    loss_kl4_mean = np.mean(loss_kl4_list)
...
def val(model, val_loader, learning, my_loss, optimizer, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(val_loader):
            ...
            S_summ_graph_list.append(result_dic['S_summ_graph'].detach().cpu().numpy())
            S_spec_graph_list.append(result_dic['S_spec_graph'].detach().cpu().numpy())
            P_summ_graph_list.append(result_dic['P_summ_graph'].detach().cpu().numpy())
            P_spec_graph_list.append(result_dic['P_spec_graph'].detach().cpu().numpy())
    ...
    loss_kl3_mean = np.mean(loss_kl3_list)
    loss_kl4_mean = np.mean(loss_kl4_list)
    S_summ_graph_list = np.squeeze(np.concatenate(S_summ_graph_list))
    S_spec_graph_list = np.squeeze(np.concatenate(S_spec_graph_list))
    P_summ_graph_list = np.squeeze(np.concatenate(P_summ_graph_list))
    P_spec_graph_list = np.squeeze(np.concatenate(P_spec_graph_list))
