...
class RTN(nn.Module):
    def __init__(self, h_dim, graph_node_dim, window_size, dropout, res=False):
        """
            h_dim: input feature dimension
            window_size: The number of nodes in the relation graph
        ...
        if self.res:
            ...
            self.gat1 = GCN(self.h_dim, self.h_dim//2, num_node=window_size, input_vector=True)
            self.gat2 = GCN(self.h_dim//2, self.h_dim//2, num_node=window_size, input_vector=False)
    ...
    def forward(self, x):
        '''
            x: (B,C,F_h) ([512,6,256] in Spatial relation inference for ISRUC)
        '''
        ...
        if self.res:
            ...
            #attention 
            H, A = self.gat1(x, alpha_bar)    # (B,C,F_h/2)
            H = self.gat2(H, A)               # (B,C,F_h/2)
            #print(H.shape)
            #print("=" * 128)
